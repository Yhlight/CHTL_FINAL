...
private:
    ...
    std::string processTemplates(const std::string& input) {
        ...
        // 处理 [Template] 块
        std::regex templateRegex(R"(\[Template\]\s+@(\w+)\s+(\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processCustoms(const std::string& input) {
        ...
        // 处理 [Custom] 块
        std::regex customRegex(R"(\[Custom\]\s+@(\w+)\s+(\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processOrigins(const std::string& input) {
        ...
        // 处理 [Origin] 块
        std::regex originRegex(R"(\[Origin\]\s+@(\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processImports(const std::string& input) {
        ...
        // 处理 [Import] 块
        std::regex importRegex(R"(\[Import\]\s+"([^"]+)"(?:\s+as\s+(\w+))?)");
        std::smatch match;
    ...
    std::string processConstraints(const std::string& input) {
        ...
        // 处理 [Constraints] 块
        std::regex constraintRegex(R"(\[Constraints\]\s+except\s+([\s\S]*?)\{)");
        std::smatch match;
    ...
    std::string processConfigurations(const std::string& input) {
        ...
        // 处理 [Configuration] 块
        std::regex configRegex(R"(\[Configuration\]\s+(\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processNamespaces(const std::string& input) {
        ...
        // 处理 [Namespace] 块
        std::regex namespaceRegex(R"(\[Namespace\]\s+(\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processElements(const std::string& input) {
        ...
        std::regex elementRegex(R"((\w+)\s*\{([\s\S]*?)\})");
        std::smatch match;
    ...
    std::string processStyles(const std::string& input) {
        ...
        // 处理 style 块
        std::regex styleRegex(R"(style\s*\{([\s\S]*?)\})");
        std::smatch match;
