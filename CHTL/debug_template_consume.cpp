...
int main() {
    ...
    auto consume = [&](CHTL::CHTLTokenType type, const std::string& errorMessage) -> bool {
        if (!match(type)) {
            std::cout << "  consume错误: " << errorMessage << "，当前token: " << current().toString() << std::endl;
            return false;
        }
        advance();
        return true;
    ...
    if (match(CHTL::CHTLTokenType::IDENTIFIER)) {
        ...
        if (match(CHTL::CHTLTokenType::COLON)) {
            ...
            if (match(CHTL::CHTLTokenType::IDENTIFIER)) {
                ...
                std::cout << "  当前token: " << current().toString() << std::endl;
                if (consume(CHTL::CHTLTokenType::SEMICOLON, "期望 ';'")) {
                    std::cout << "  consume SEMICOLON 成功" << std::endl;
                } else {
                    std::cout << "  consume SEMICOLON 失败" << std::endl;
                    return 1;
                }
            } else {
