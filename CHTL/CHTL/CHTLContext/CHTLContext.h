...
public:
    CHTLContext();
    CHTLContext(const std::string& source, const std::string& output);
    virtual ~CHTLContext() = default;
    ...
    // 克隆
    CHTLContext clone() const;
    
    // 比较
    bool operator==(const CHTLContext& other) const;
    bool operator!=(const CHTLContext& other) const;
};
