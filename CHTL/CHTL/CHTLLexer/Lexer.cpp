...
char Lexer::peekNext() const { if (current + 1 >= source.length()) return '\0'; return source[current + 1]; }
bool Lexer::isAtEnd() const { return current >= source.length(); }
Token Lexer::makeToken(TokenType type, const std::string& value) { return Token{type, value, line, column - value.length()}; }
Token Lexer::makeToken(TokenType type, char value) { return Token{type, std::string(1, value), line, column - 1}; }
...
Token Lexer::scanIdentifierOrKeyword() {
    ...
    if (value == "Import") return makeToken(TokenType::KEYWORD_IMPORT, value);
    if (value == "from") return makeToken(TokenType::KEYWORD_FROM, value);
    if (value == "as") return makeToken(TokenType::KEYWORD_AS, value);
