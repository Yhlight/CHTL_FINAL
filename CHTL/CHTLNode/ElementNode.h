...
struct HtmlAttribute {
    ...
    HtmlAttribute() : value_expr(nullptr) {}
    HtmlAttribute(const std::string& k, const std::string& v) : key(k), value(v), value_expr(nullptr) {}
    HtmlAttribute(const std::string& k, std::unique_ptr<Expr> expr) : key(k), value(""), value_expr(std::move(expr)) {}
    
    // Copy constructor
    HtmlAttribute(const HtmlAttribute& other) 
        : key(other.key), value(other.value), value_expr(other.value_expr ? other.value_expr->clone() : nullptr) {}
    ...
    HtmlAttribute(HtmlAttribute&& other) noexcept 
        : key(std::move(other.key)), value(std::move(other.value)), value_expr(std::move(other.value_expr)) {}
