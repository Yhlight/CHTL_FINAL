...
// --- Test Cases (with corrected expectations for style spacing) ---
...
void test_conditional_true() { run_test("Phase 2.B: Conditional (True)", "div{ style{ width:100px; color: width > 50px ? 'green' : 'red'; } }", "<div style=\"width: 100px;color: green;\"></div>"); }
void test_conditional_false() { run_test("Phase 2.B: Conditional (False)", "div{ style{ width:40px; color: width > 50px ? 'green' : 'red'; } }", "<div style=\"width: 40px;color: red;\"></div>"); }
void test_conditional_logical_and() { run_test("Phase 2.B: Conditional (Logical AND)", "div{ style{ width:60px; height:80px; color: width > 50px && height < 90px ? 'pass' : 'fail'; } }", "<div style=\"width: 60px;height: 80px;color: pass;\"></div>"); }
void test_conditional_logical_or() { run_test("Phase 2.B: Conditional (Logical OR)", "div{ style{ width:40px; height:80px; color: width > 50px || height < 90px ? 'pass' : 'fail'; } }", "<div style=\"width: 40px;height: 80px;color: pass;\"></div>"); }
void test_simple_element_template() { run_test("Phase 3: Simple Element Template", "[Template] @Element Box { div{} } body { @Element Box; }", "<body><div></div></body>"); }
void test_nested_element_template() { run_test("Phase 3: Nested Element Template", "[Template] @Element Inner{ span{} } [Template] @Element Outer{ div{ @Element Inner; } } body{ @Element Outer; }", "<body><div><span></span></div></body>"); }
