...
private:
    // This private method is part of the old implementation and will be removed
    // during the refactoring in a later step.
    // void process_source(const std::string& source, int depth);

    std::string initial_source;
    std::map<std::string, std::shared_ptr<TemplateDefinitionNode>> all_template_definitions;
