...
NodePtr CHTLParser::ParseStyleBlock(bool isGlobal) {
    ...
    while (Peek().type != TokenType::CloseBrace && Peek().type != TokenType::EndOfFile) {
        ...
        else if (token1.type == TokenType::Identifier && Peek(1).type == TokenType::Colon) {
            Token propName = Consume(); Consume();
            ExpressionNodePtr propValue = ParseExpression(LOWEST);
...
NodePtr CHTLParser::ParseTemplateDefinition() {
    ...
    Token templateName = Expect(TokenType::Identifier);
    TemplateType type;
    if (templateTypeToken.value == "Style") type = TemplateType::Style;
    else if (templateTypeToken.value == "Element") type = TemplateType::Element;
    else if (templateTypeToken.value == "Var") type = TemplateType::Var;
    ...
    Expect(TokenType::CloseBrace);
    return std::make_shared<TemplateDefinitionNode>(templateName.value, type, content, false);
}
