...
std::shared_ptr<StyleNode> Parser::parseStyleNode() {
    ...
    while (!currentTokenIs(TokenType::RIGHT_BRACE) && !currentTokenIs(TokenType::END_OF_FILE)) {
        if (currentTokenIs(TokenType::DOT) || currentTokenIs(TokenType::POUND) || currentTokenIs(TokenType::AMPERSAND)) {
            // It's a nested rule
            ...
            if (!currentTokenIs(TokenType::LEFT_BRACE)) {
                ...
                // Skip to next potential statement to avoid cascade failure
            ...
            nextToken(); // Skip comments, extra semicolons, etc.
        }
    }

    // If we collected any inline properties, add them as a rule with selector "&"
    if (!inlineProps.empty()) {
        auto inlineRule = std::make_shared<CssRuleNode>("&");
        inlineRule->properties = inlineProps;
        // Add to the front so they are processed first by the generator
        node->rules.insert(node->rules.begin(), inlineRule);
    }
