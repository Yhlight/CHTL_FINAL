...
// --- Helper Functions ---
// ... (omitting unchanged helpers for brevity)
std::string ValueToString(const EvaluatedValue& val) {
    if (val.type == ValueType::String) return std::get<std::string>(val.value);
    if (val.type == ValueType::Number) { std::stringstream ss; ss << std::get<double>(val.value); return ss.str() + val.unit; }
    if (val.type == ValueType::Bool) return std::get<bool>(val.value) ? "true" : "false";
    return "";
}
std::string CHTLGenerator::escapeHTML(const std::string& data) {
    std::string buffer; buffer.reserve(data.size());
...
std::string CHTLGenerator::Generate(const NodeList& ast) {
    m_output_body.str(""); m_output_css.str(""); m_template_repo.clear();
...
NodePtr CHTLGenerator::FindNodeBySelector(NodeList& nodes, const std::string& selector) {
    // This is a simplified selector logic, only handles tag name and index.
...
void CHTLGenerator::ApplyModification(NodeList& nodes, const ElementModificationNode* mod) {
    ...
    auto targetElement = static_cast<ElementNode*>(targetNode.get());
...
void CHTLGenerator::ApplyDeletion(NodeList& nodes, const ElementDeleteNode* del) {
    for (const auto& selector : del->GetSelectors()) {
        nodes.erase(std::remove_if(nodes.begin(), nodes.end(), [&](const NodePtr& node) {
            if (node->GetType() == NodeType::Element) {
                // This is a simplified check. A real implementation would need to parse the selector properly.
...
void CHTLGenerator::ApplyInsertion(NodeList& nodes, const ElementInsertNode* ins) {
    // Simplified: does not handle all positions or complex selectors yet.
...
void CHTLGenerator::VisitElementTemplateUsage(const ElementTemplateUsageNode* node) {
    ...
    // 3. Visit the modified AST to render it
    for (const auto& contentNode : clonedContent) {
        Visit(contentNode);
    }
}

// ... (omitting unchanged VisitElement, VisitText, VisitComment, style/expression helpers for brevity)
...
EvaluatedValue CHTLGenerator::VisitVariableUsage(const VariableUsageNode* node, const ElementNode* context) {
    ...
    const auto* styleNode = static_cast<const StyleNode*>(templateDef->GetContent().front().get());
    for (const auto& prop : styleNode->GetProperties()) {
        if (prop.name == node->GetVariableName()) return EvaluateExpression(prop.value, nullptr);
    }
