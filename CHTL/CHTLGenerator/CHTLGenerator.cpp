...
CHTLGenerator::CHTLGenerator(const std::map<std::string, std::map<std::string, TemplateDefinitionNode>>& templates)
    ...
    js_compiler = std::make_unique<CHTL_JS::CHTLJSCompiler>();
}

CHTLGenerator::~CHTLGenerator() = default;

#include "../../CHTL JS/CHTLJSCompilationResult.h"
...
void CHTLGenerator::visit(ScriptNode& node) {
    CHTL_JS::CHTLJSCompilationResult js_result = js_compiler->compile(node.content);
    for (const auto& delegate_node : js_result.delegate_nodes) {
        delegate_registry[delegate_node.parent_selector.selector_string].push_back(delegate_node);
    }
    js_output << js_result.js;
}
