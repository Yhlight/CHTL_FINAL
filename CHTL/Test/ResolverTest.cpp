...
public:
    std::string print(std::shared_ptr<RootNode> root) { if (root) root->accept(*this); return result; }
    ...
    void visit(PropertyNode& node) override { result += std::string(indent*2, ' ') + "(Property " + node.key + (node.value ? " = ..." : " = no_value") + ")\n"; }
