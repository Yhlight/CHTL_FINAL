...
// NewParserV2 创建改进的语法分析器
func NewParserV2(lexer *lexer.Lexer) *ParserV2 {
	p := &ParserV2{
		lexer:  lexer,
...
// ParseProgram 解析程序
func (p *ParserV2) ParseProgram() *node.ProgramNode {
...
// parseUse 解析使用声明
func (p *ParserV2) parseUse() *node.UseNode {
...
// parseImport 解析导入
func (p *ParserV2) parseImport() *node.ImportNode {
	pos := p.currentToken.Position
	p.nextToken()
...
func (p *ParserV2) parseNamespace() *node.NamespaceNode {
	pos := p.currentToken.Position
	p.nextToken()
...
func (p *ParserV2) parseConfiguration() *node.ConfigurationNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// parseExcept 解析约束
func (p *ParserV2) parseExcept() *node.ExceptNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// parseElement 解析元素
func (p *ParserV2) parseElement() *node.ElementNode {
	pos := p.currentToken.Position
	...
	if p.currentToken.Type == lexer.LEFT_BRACE {
		...
		for p.currentToken.Type != lexer.RIGHT_BRACE && p.currentToken.Type != lexer.EOF {
			...
			case lexer.IDENTIFIER:
				// 检查是否为属性
				if p.peekToken.Type == lexer.COLON || p.peekToken.Type == lexer.EQUALS {
...
// parseAttribute 解析属性
func (p *ParserV2) parseAttribute() *node.AttributeNode {
	pos := p.currentToken.Position
	name := p.currentToken.Literal
	p.nextToken()
...
// parseText 解析文本
func (p *ParserV2) parseText() *node.TextNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// parseStyle 解析样式
func (p *ParserV2) parseStyle() *node.StyleNode {
	pos := p.currentToken.Position
	p.nextToken()
...
func (p *ParserV2) parseStyleProperty() *node.StyleProperty {
	pos := p.currentToken.Position
	name := p.currentToken.Literal
	p.nextToken()
...
func (p *ParserV2) parseStyleSelector() *node.StyleSelector {
	pos := p.currentToken.Position
	var selector string
...
// parseScript 解析脚本
func (p *ParserV2) parseScript() *node.ScriptNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// parseTemplate 解析模板
func (p *ParserV2) parseTemplate() *node.TemplateNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// parseCustom 解析自定义
func (p *ParserV2) parseCustom() *node.CustomNode {
	pos := p.currentToken.Position
	p.nextToken()
...
// addError 添加错误
func (p *ParserV2) addError(message string) {
