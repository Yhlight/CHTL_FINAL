...
void CHTLUnifiedScanner::scanSource() {
    ...
    if (opening_brace == std::string::npos) {
        m_cursor = next_block_pos + 6; // Move past "script" or "style"
        return;
    }
...
void CHTLUnifiedScanner::scanScriptContent(size_t block_end) {
    ...
    while (m_cursor < block_end) {
        // Look for CHTL JS constructs
        size_t next_chtl_js_pos = findNextCHTLJSConstruct(m_cursor, block_end);
        
        if (next_chtl_js_pos == std::string::npos || next_chtl_js_pos >= block_end) {
        ...
        if (next_chtl_js_pos > m_cursor) {
            js_buffer += m_source.substr(m_cursor, next_chtl_js_pos - m_cursor);
        }

        finalize_js_buffer();

        // Process the CHTL JS construct
        m_cursor = next_chtl_js_pos;
        size_t construct_start = m_cursor;

        // Find the end of the CHTL JS construct
        size_t end_pos = findEndOfCHTLJSConstruct(m_cursor, block_end);
...
size_t CHTLUnifiedScanner::findNextCHTLJSConstruct(size_t start_pos, size_t end_pos) {
    ...
    // Look for CHTL JS keywords
    for (const auto& keyword : CHTLJS_KEYWORDS) {
        size_t pos = findNextKeyword(keyword, start_pos);
        if (pos != std::string::npos && pos < end_pos) {
            if (next_pos == std::string::npos) {
                next_pos = pos;
            } else {
    ...
    // Look for {{ }} patterns
    size_t open_brace_pos = m_source.find("{{", start_pos);
    if (open_brace_pos != std::string::npos && open_brace_pos < end_pos) {
        if (next_pos == std::string::npos) {
            next_pos = open_brace_pos;
        } else {
            next_pos = std::min(next_pos, open_brace_pos);
        }
    }
...
size_t CHTLUnifiedScanner::findEndOfCHTLJSConstruct(size_t start_pos, size_t end_pos) {
    // For CHTL JS constructs, we need to find the matching brace
    size_t open_brace = m_source.find('{', start_pos);
    if (open_brace != std::string::npos && open_brace < end_pos) {
        size_t close_brace = findMatchingBrace(open_brace);
        if (close_brace != std::string::npos && close_brace < end_pos) {
            return close_brace + 1;
        }
    }
    
    // If no braces, look for semicolon or comma
    return findEndOfCHTLFeature(start_pos, end_pos);
}
