extern crate chtl;
use chtl::{StyleBlock, StyleValue, StyleSystem, ContextRule, AttributeCondition, AttributeOperator};
...
fn main() {
    ...
    // Add some properties
    style_block.add_property("background-color".to_string(), StyleValue::Literal("blue".to_string()));
    style_block.add_property("color".to_string(), StyleValue::Literal("white".to_string()));
    style_block.add_property("padding".to_string(), StyleValue::Literal("10px".to_string()));
    
    // Add a context rule for hover
    let mut hover_rule = ContextRule {
        selector: "hover".to_string(),
        properties: vec![
            ("background-color".to_string(), StyleValue::Literal("darkblue".to_string())),
        ],
        priority: 1,
    };
    ...
    let mut disabled_condition = AttributeCondition {
        ...
        properties: vec![
            ("opacity".to_string(), StyleValue::Literal("0.5".to_string())),
            ("cursor".to_string(), StyleValue::Literal("not-allowed".to_string())),
        ],
    };
