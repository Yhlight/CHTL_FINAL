...
CHTLIOStream::CHTLIOStream() : fileStream(std::make_unique<std::fstream>()) {}

CHTLIOStream::~CHTLIOStream() {
    close();
}

bool CHTLIOStream::open(const std::string& filePath, std::ios_base::openmode mode) {
    close();
...
bool CHTLIOStream::write(const std::string& content) {
    if (!isOpen()) {
        return false;
    }
...
size_t CHTLIOStream::getFileSize() const {
    if (!isOpen()) {
        return 0;
    }
...
bool CHTLIOStream::fileExists(const std::string& filePath) {
    return std::filesystem::exists(filePath);
}

bool CHTLIOStream::createDirectory(const std::string& dirPath) {
    try {
