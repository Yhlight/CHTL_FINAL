        ...
        for (const auto& fragment : fragments) {
            if (fragment.type == CodeFragmentType::CHTL) {
                ...
                if (ast) {
                    ...
                    generator.setGenerateCSS(!noCSS);
                    generator.setGenerateJavaScript(!noJS);
                    generator.setGenerateDefaultStructure(defaultStruct);
                    
                    generatedHTML = generator.generateHTML(ast);
                    generatedCSS = generator.generateCSS(ast);
                    generatedJS = generator.generateJavaScript(ast);
                    
                    if (verboseMode) {
                        std::cout << "Code generation completed" << std::endl;
                        std::cout << "HTML length: " << generatedHTML.length() << std::endl;
                        std::cout << "CSS length: " << generatedCSS.length() << std::endl;
                        std::cout << "JS length: " << generatedJS.length() << std::endl;
                    }
                } else {
                    std::cerr << "Error: Failed to parse CHTL fragment" << std::endl;
                }
            }
        }
