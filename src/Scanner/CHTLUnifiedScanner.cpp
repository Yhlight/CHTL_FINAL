...
bool CHTLUnifiedScanner::isWordBoundary(size_t pos, size_t word_len) const {
    if (pos > 0 && (isalnum(source_[pos - 1]) || source_[pos - 1] == '_')) {
        return false; // Not a start of a word
    }
    size_t after_pos = pos + word_len;
    if (after_pos < source_.length() && (isalnum(source_[after_pos]) || source_[after_pos] == '_')) {
...
size_t CHTLUnifiedScanner::findMatchingBrace(size_t start_pos) const {
...
std::vector<CodeChunk> CHTLUnifiedScanner::scan() {
    ...
    while (current_ < source_.length()) {
        ...
        bool handled = false;
        if (next_pos == script_pos && isWordBoundary(script_pos, 6)) {
            handled = handleScriptBlock(script_pos, last_pos);
        } else if (next_pos == style_pos && isWordBoundary(style_pos, 5)) {
...
bool CHTLUnifiedScanner::handleScriptBlock(size_t keyword_pos, size_t& last_pos) {
    ...
    size_t brace_close_pos = findMatchingBrace(brace_open_pos);
