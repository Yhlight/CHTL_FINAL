...
int main() {
    std::string entry_file = "Test/ArithmeticTest/test.chtl";
    std::cout << "--- Running Arithmetic Test: " << entry_file << " ---\n" << std::endl;
    ...
    try {
        ...
        // We are interested in the inline style attribute generated for the div
        ...
        std::cout << "Expected Style:  " << expected_style << std::endl;

        if (normalized_generated == normalized_expected) {
            std::cout << "\n--- Arithmetic Test PASSED ---" << std::endl;
        } else {
