...
class TestEvaluator(unittest.TestCase):
    ...
    def test_arithmetic_evaluation(self):
        prop_node = StylePropertyNode(
            "prop", BinaryOpNode(DimensionNode("10", "px"), Token(TokenType.PLUS, '+', 1, 1), DimensionNode("5", "px"))
        )
        evaluator = Evaluator([prop_node], self.element_registry)
    ...
    def test_dependency_resolution(self):
        ...
        height_node = StylePropertyNode(
            "height", BinaryOpNode(IdentifierNode("width"), Token(TokenType.STAR, '*', 1, 1), DimensionNode("2", ""))
        )
        evaluator = Evaluator([width_node, height_node], self.element_registry)
    ...
    def test_conditional_with_dependency(self):
        width_node = StylePropertyNode("width", DimensionNode("100", "px"))
        color_node = StylePropertyNode(
            "color",
            ConditionalNode(
                BinaryOpNode(IdentifierNode("width"), Token(TokenType.GREATER, '>', 1, 1), DimensionNode("50", "px")),
                IdentifierNode("red"),
                IdentifierNode("blue")
            )
        )
        evaluator = Evaluator([width_node, color_node], self.element_registry)
    ...
    def test_cycle_detection(self):
        ...
        evaluator = Evaluator([prop1, prop2], self.element_registry)
        with self.assertRaisesRegex(Exception, "Circular dependency detected"):
