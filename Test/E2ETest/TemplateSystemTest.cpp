...
int main() {
    std::ifstream file("Test/E2ETest/TemplateSystem.chtl");
    if (!file) {
        std::cerr << "Failed to open test file 'Test/E2ETest/TemplateSystem.chtl'" << std::endl;
        return 1;
    }
    ...
    std::cout << "--- CHTL Source (TemplateSystem) ---" << std::endl;
    ...
    std::string expected_html = R"(
        <body>
            <div class="card-wrapper">
                <h2 style="font-weight:bold;font-size:24px;color:blue;">
                    A Card Title
                </h2>
                <p>
                    Some card content.
    ...
    try {
        ...
        std::cout << "SUCCESS: Generated HTML matches expected HTML for template system." << std::endl;
        return 0;
