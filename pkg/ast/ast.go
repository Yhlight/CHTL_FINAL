...
func (p *Program) String() string { var out strings.Builder; for _, s := range p.Statements { out.WriteString(s.String()) }; return out.String() }
...
func (bs *BlockStatement) String() string { var out strings.Builder; out.WriteString(" { "); for _, s := range bs.Statements { out.WriteString(s.String()) }; out.WriteString(" }"); return out.String() }
...
func (td *TemplateDefinition) String() string       { return "[Template] @" + td.TemplateType + " " + td.Name.String() + td.Body.String() }
...
func (re *RawExpression) TokenLiteral() string { if len(re.Tokens) > 0 { return re.Tokens[0].Literal }; return "" }
